...
c.....Generate the log-normal distribution of particle radii
      ...
      WRITE(*,*) 'The sample height = ', height
      WRITE(*,*) 'Num spheres in the initial packing = ', npart
...
c.....Initialize
      ...
      ENDDO
      radius = EXP(sample)
      radminus = (radius-rmin)/(delr)
            DO k = 1, mr
                  IF( radminus.GE.(k-1).AND.radminus.LT.(k)) THEN
...
c    				write(*,*) i1x,jy,kz,Pfun(i1x,jy,kz)
            ENDDO
         ENDDO
      ENDDO
...
c.....Write particles in file after allocation
      ...
      DO jy = 1, nc
            DO i1x = 1, nc
               ncpart = icnt(i1x,jy)
					WRITE(9,2020) i1x, jy, ncpart
c					WRITE(9,*) (xc(i1x,jy,ic), yc(i1x,jy,ic), zc(i1x,jy,ic), 
c     1                     rc(i1x,jy,ic), ic = 1, ncpart) 
				   DO ic = 1, ncpart
						WRITE(9,2030) xc(i1x,jy,ic), yc(i1x,jy,ic),
...
10    CALL FindXYZ
      ...
      IF(z1.GE.bound.AND.x1.GE.bound.AND.y1.GE.bound) THEN
            IF(x1.LE.(size-bound).AND.y1.LE.(size-bound)) THEN
                  CALL FindLoc(lz)
...
c------------------------------------------------------------------------
     ...
     1               niterate
      ...
      IF(kz1.EQ.1.OR.kz1.EQ.(ncz+1)) THEN
            IF(i1x1.EQ.1.AND.jy1.EQ.1) THEN
                  CALL Shift1(i1xx,jyy,kzz,mflag)
            ELSEIF(i1x1.EQ.nc.AND.jy1.EQ.1) THEN
                  CALL Shift1(i1xx,jyy,kzz,mflag)
            ELSEIF(i1x1.EQ.1.AND.jy1.EQ.nc) THEN
                  CALL Shift1(i1xx,jyy,kzz,mflag)
            ELSEIF(i1x1.EQ.nc.AND.jy1.EQ.nc) THEN
                  CALL Shift1(i1xx,jyy,kzz,mflag)
...
c------------------------------------------------------------------------
     ...
     1               niterate
      ...
      DO icst = 1, ncntst
            IF(.NOT.(i1x.EQ.i1x2.AND.jy.EQ.jy2)) THEN
              IF(.NOT.(kz.EQ.kz2.AND.icst.EQ.icst2)) THEN
                  CALL CalR1Loc(icst)
...
c------------------------------------------------------------------------
      ...
      lshift = lshift+1
10    END
c CLT: what does this mean?
c10		END
c rnpack.f(2626): error #5082: Syntax error, found INTEGER_CONSTANT '10' when expecting one of: <LABEL> <END-OF-STATEMENT> ; <IDENTIFIER> TYPE MODULE ELEMENTAL IMPURE NON_RECURSIVE...
c------------------------------------------------------------------------
        ...
        DO jy = ncl, nch
          DO i1x = ncl, nch
            ...
            DO icst = 1, ncntst
              ...
              r1 = rcst(i1x,jy,kz,icst)
      IF(x1.GE.tolrnce.AND.x1.LE.(size+tolrnce).AND.y1.GE.tolrnce) THEN
            IF(y1.LE.(size+tolrnce)) THEN
                  IF(z1.GE.tolrnce) THEN
                        IF(z1.LE.(height+tolrnce)) THEN
                              npart = npart+1
                              ic = ic+1
                              x(ic) = x1
                              y(ic) = y1
