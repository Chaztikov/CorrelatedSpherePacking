/* rnpack.f -- translated by f2c (version 20160102).
...
struct {
    real r__[100000], rc[8000000]	/* was [200][200][200] */, x[100000], 
	    xc[8000000]	/* was [200][200][200] */, y[100000], yc[8000000]	
	    /* was [200][200][200] */, z__[100000], zc[8000000]	/* was [200][
	    200][200] */;
    integer icnt[40000]	/* was [200][200] */;
} origdim_;
...
struct {
    integer icst2, icst3, icst4, i1x, i1x1, i1x2, i1x3, i1x4, jy, jy1, jy2, 
	    jy3, jy4, kz, kz1, kz2, kz3, kz4;
...
struct {
    real rcst[1600000000]	/* was [200][200][200][200] */, xcst[
	    1600000000]	/* was [200][200][200][200] */, ycst[1600000000]	
	    /* was [200][200][200][200] */, zcst[1600000000]	/* was [200][
	    200][200][200] */;
    integer icntst[8000000]	/* was [200][200][200] */;
} sortdim_;
...
struct {
    ...
    integer i1xshift[30], jyshift[30], kzshift[30], indx[30];
} shift_;
...
/* .....Generate the log-normal distribution of particle radii */
    ...
    e_wsle();
    s_wsle(&io___24);
    do_lio(&c__9, &c__1, "The sample height = ", (ftnlen)20);
    do_lio(&c__4, &c__1, (char *)&param_1.height, (ftnlen)sizeof(real));
    e_wsle();
    s_wsle(&io___25);
    do_lio(&c__9, &c__1, "Num spheres in the initial packing = ", (ftnlen)37);
...
doublereal rinvnorm_(real *p)
{
    ...
    static integer ka;
    static real ord, apprxl, apprxu, thenord;
...
/* Subroutine */ int gnrtpack_(void)
{
    ...
    static real radminus;
    extern doublereal rinvnorm_(real *);
    static integer k, l;
    static real pd, pf;
    static integer jy, kz;
    static real vt;
    static integer i1x;
    extern doublereal ran_(integer *);
    ...
    static integer isample, nradius[100];
    static real rminlog, rmaxlog, xsample, ysample, zsample;

    /* Fortran I/O blocks */
    static cilist io___74 = { 0, 12, 0, fmt_2000, 0 };
    static cilist io___75 = { 0, 4, 0, fmt_1000, 0 };
    static cilist io___79 = { 0, 4, 0, fmt_1010, 0 };
...
/* .....Initialize */
	...
	radius = exp(sample);
	radminus = (radius - dist_1.rmin) / delr;
	i__1 = corr_1.mr;
	for (k = 1; k <= i__1; ++k) {
	    if (radminus >= (real) (k - 1) && radminus < (real) k) {
		++nradius[k - 1];
	    }
	}
    }
...
/* .....Read in the random P-field data */
	...
	for (jy = 1; jy <= i__2; ++jy) {
	    i__3 = corr_1.ngrid;
	    for (i1x = 1; i1x <= i__3; ++i1x) {
		s_rsfe(&io___79);
		do_fio(&c__1, (char *)&pfun[i1x + (jy + kz * 100) * 100 - 
			10101], (ftnlen)sizeof(real));
		e_rsfe();
/*    				write(*,*) i1x,jy,kz,Pfun(i1x,jy,kz) */
	    }
	}
    }
/* .....Create the initial random packing with spatial correlations */
	...
	pf = pfun[i1x + (jy + kz * 100) * 100 - 10101];
	if (pf < cpdf[0]) {
    ...
    e_wsle();
    param_1.bound = param_1.cmptln - dist_1.rmax * 2.f;
/*      WRITE(*,*) "Do you want to start rearranging process(1--Yes, 0--No)?" */
/*      READ(*,*) lsignal */
...
/* Subroutine */ int output_(void)
{
    ...
    /* Fortran I/O blocks */
    static cilist io___93 = { 0, 8, 0, fmt_2010, 0 };
...
/* Subroutine */ int alloctcmpt_(void)
{
    ...
    static char fmt_2000[] = "(\002npart_kz\002,/,\002i1x  jy  ncpart\002,/"
	    ",\002xc  yc  zc  rc\002)";
    ...
    /* Fortran I/O blocks */
    static cilist io___95 = { 0, 8, 0, fmt_1000, 0 };
    static cilist io___96 = { 0, 9, 0, fmt_2000, 0 };
    static cilist io___98 = { 0, 9, 0, fmt_2010, 0 };
    static cilist io___100 = { 0, 9, 0, fmt_2020, 0 };
    static cilist io___102 = { 0, 9, 0, fmt_2030, 0 };
...
/* .....Sum up the total counts in the layer */
	...
	for (data_1.jy = 1; data_1.jy <= i__2; ++data_1.jy) {
	    i__3 = param_1.nc;
	    for (data_1.i1x = 1; data_1.i1x <= i__3; ++data_1.i1x) {
		param_1.npart_kz__ += origdim_1.icnt[data_1.i1x + data_1.jy * 
			200 - 201];
	    }
	}
/* .....Write particles in file after allocation */
	...
	for (data_1.jy = 1; data_1.jy <= i__2; ++data_1.jy) {
	    i__3 = param_1.nc;
	    for (data_1.i1x = 1; data_1.i1x <= i__3; ++data_1.i1x) {
		ncpart = origdim_1.icnt[data_1.i1x + data_1.jy * 200 - 201];
		s_wsfe(&io___100);
		do_fio(&c__1, (char *)&data_1.i1x, (ftnlen)sizeof(integer));
		do_fio(&c__1, (char *)&data_1.jy, (ftnlen)sizeof(integer));
		do_fio(&c__1, (char *)&ncpart, (ftnlen)sizeof(integer));
		e_wsfe();
/* 					WRITE(9,*) (xc(i1x,jy,ic), yc(i1x,jy,ic), zc(i1x,jy,ic), */
/*     1                     rc(i1x,jy,ic), ic = 1, ncpart) */
		i__4 = ncpart;
		for (ic = 1; ic <= i__4; ++ic) {
		    s_wsfe(&io___102);
		    do_fio(&c__1, (char *)&origdim_1.xc[data_1.i1x + (
			    data_1.jy + ic * 200) * 200 - 40201], (ftnlen)
			    sizeof(real));
		    do_fio(&c__1, (char *)&origdim_1.yc[data_1.i1x + (
			    data_1.jy + ic * 200) * 200 - 40201], (ftnlen)
			    sizeof(real));
		    do_fio(&c__1, (char *)&origdim_1.zc[data_1.i1x + (
			    data_1.jy + ic * 200) * 200 - 40201], (ftnlen)
			    sizeof(real));
		    do_fio(&c__1, (char *)&origdim_1.rc[data_1.i1x + (
			    data_1.jy + ic * 200) * 200 - 40201], (ftnlen)
			    sizeof(real));
		    e_wsfe();
		}
...
/* .....Initiallize counts in each compartment */
	...
	i__2 = param_1.nc;
	for (data_1.i1x = 1; data_1.i1x <= i__2; ++data_1.i1x) {
	    origdim_1.icnt[data_1.i1x + data_1.jy * 200 - 201] = 0;
	}
    }
...
/* Subroutine */ int allocating_(integer *i__, integer *mz)
{
	...
	for (data_1.i1x = 1; data_1.i1x <= i__2; ++data_1.i1x) {
	    if (origdim_1.x[*i__ - 1] >= mx * param_1.cmptln && origdim_1.x[*
		     ...
		     * param_1.cmptln) {
		++origdim_1.icnt[data_1.i1x + data_1.jy * 200 - 201];
		ic = origdim_1.icnt[data_1.i1x + data_1.jy * 200 - 201];
		origdim_1.xc[data_1.i1x + (data_1.jy + ic * 200) * 200 - 
			40201] = origdim_1.x[*i__ - 1];
		origdim_1.yc[data_1.i1x + (data_1.jy + ic * 200) * 200 - 
			40201] = origdim_1.y[*i__ - 1];
		origdim_1.zc[data_1.i1x + (data_1.jy + ic * 200) * 200 - 
			40201] = origdim_1.z__[*i__ - 1];
		origdim_1.rc[data_1.i1x + (data_1.jy + ic * 200) * 200 - 
			40201] = origdim_1.r__[*i__ - 1];
	    }
	    ++mx;
	}
	++my;
...
/* Subroutine */ int sort_(void)
{
    ...
    /* Fortran I/O blocks */
    static cilist io___107 = { 0, 9, 0, fmt_1000, 0 };
    static cilist io___108 = { 0, 9, 0, 0, 0 };
    static cilist io___109 = { 0, 9, 0, 0, 0 };
    static cilist io___113 = { 0, 9, 0, fmt_1010, 0 };
...
/* .....Sort particles layer by layer */
	...
	for (data_1.jy = 1; data_1.jy <= i__2; ++data_1.jy) {
		...
		do_lio(&c__3, &c__1, (char *)&ncpart, (ftnlen)sizeof(integer))
			;
		e_rsle();
		origdim_1.icnt[data_1.i1x + data_1.jy * 200 - 201] = ncpart;
		i__4 = ncpart;
		for (ic = 1; ic <= i__4; ++ic) {
		    s_rsfe(&io___113);
		    do_fio(&c__1, (char *)&origdim_1.xc[data_1.i1x + (
...
/* .....Initialize counts in a layer of lz after sort */
	...
	i__2 = param_1.nc;
	for (data_1.i1x = 1; data_1.i1x <= i__2; ++data_1.i1x) {
	    sortdim_1.icntst[data_1.i1x + (data_1.jy + *lz * 200) * 200 - 
		    40201] = 0;
	}
    }
    return 0;
...
/* Subroutine */ int findzmin_(integer *ic1, integer *ncpart1)
{
	...
	for (data_1.i1x = 1; data_1.i1x <= i__2; ++data_1.i1x) {
	    ...
	    i__3 = ncpart;
	    for (ic = 1; ic <= i__3; ++ic) {
		if (origdim_1.zc[data_1.i1x + (data_1.jy + ic * 200) * 200 - 
			40201] < data_1.z1) {
		    data_1.z1 = origdim_1.zc[data_1.i1x + (data_1.jy + ic * 
			    200) * 200 - 40201];
		    *ic1 = ic;
		    *ncpart1 = ncpart;
		    data_1.i1x1 = data_1.i1x;
...
/* Subroutine */ int reorder_(integer *ic1, integer *ncpart1)
{
    ...
    data_1.x1 = origdim_1.xc[data_1.i1x1 + (data_1.jy1 + *ic1 * 200) * 200 - 
	    40201];
    data_1.y1 = origdim_1.yc[data_1.i1x1 + (data_1.jy1 + *ic1 * 200) * 200 - 
	    40201];
    data_1.z1 = origdim_1.zc[data_1.i1x1 + (data_1.jy1 + *ic1 * 200) * 200 - 
	    40201];
    data_1.r1 = origdim_1.rc[data_1.i1x1 + (data_1.jy1 + *ic1 * 200) * 200 - 
	    40201];
    --origdim_1.icnt[data_1.i1x1 + data_1.jy1 * 200 - 201];
    i__1 = *ncpart1;
    for (ic = *ic1 + 1; ic <= i__1; ++ic) {
	origdim_1.xc[data_1.i1x1 + (data_1.jy1 + (ic - 1) * 200) * 200 - 
		40201] = origdim_1.xc[data_1.i1x1 + (data_1.jy1 + ic * 200) * 
		200 - 40201];
	origdim_1.yc[data_1.i1x1 + (data_1.jy1 + (ic - 1) * 200) * 200 - 
		40201] = origdim_1.yc[data_1.i1x1 + (data_1.jy1 + ic * 200) * 
		200 - 40201];
	origdim_1.zc[data_1.i1x1 + (data_1.jy1 + (ic - 1) * 200) * 200 - 
		40201] = origdim_1.zc[data_1.i1x1 + (data_1.jy1 + ic * 200) * 
		200 - 40201];
	origdim_1.rc[data_1.i1x1 + (data_1.jy1 + (ic - 1) * 200) * 200 - 
		40201] = origdim_1.rc[data_1.i1x1 + (data_1.jy1 + ic * 200) * 
		200 - 40201];
    }
    return 0;
} /* reorder_ */
...
L10:
    ...
    if (data_1.z1 >= param_1.bound && data_1.x1 >= param_1.bound && data_1.y1 
	    >= param_1.bound) {
	if (data_1.x1 <= param_1.size - param_1.bound && data_1.y1 <= 
	...
	if (data_1.x1 > param_1.size - param_1.bound) {
	    findlocbc_(lz);
	    if (index_1.mcount == 11) {
		goto L10;
	    }
	...
	if (data_1.y1 > param_1.size - param_1.bound) {
	    findlocbc_(lz);
	    if (index_1.mcount == 11) {
		goto L10;
	    }
...
/* Subroutine */ int findxyz_(void)
{
    ...
    /* Local variables */
    static integer jyy, kzz, i1xx, mflag;
    static real zorig;
	...
	if (data_1.kz1 == 1 || data_1.kz1 == param_1.ncz + 1) {
	    if (data_1.i1x1 == 1 && data_1.jy1 == 1) {
		shift1_(&i1xx, &jyy, &kzz, &mflag);
	    } else if (data_1.i1x1 == param_1.nc && data_1.jy1 == 1) {
		shift1_(&i1xx, &jyy, &kzz, &mflag);
	    } else if (data_1.i1x1 == 1 && data_1.jy1 == param_1.nc) {
		shift1_(&i1xx, &jyy, &kzz, &mflag);
	    } else if (data_1.i1x1 == param_1.nc && data_1.jy1 == param_1.nc) 
		    {
		shift1_(&i1xx, &jyy, &kzz, &mflag);
	    } else if (data_1.i1x1 == 1 || data_1.i1x1 == param_1.nc) {
		shift2_(&i1xx, &jyy, &kzz, &mflag);
	    } else if (data_1.jy1 == 1 || data_1.jy1 == param_1.nc) {
		shift2_(&i1xx, &jyy, &kzz, &mflag);
	    } else {
		shift3_(&i1xx, &jyy, &kzz, &mflag);
	    }
	} else if (data_1.jy1 == 1 || data_1.jy1 == param_1.nc) {
	    if (data_1.i1x1 == 1 || data_1.i1x1 == param_1.nc) {
		shift2_(&i1xx, &jyy, &kzz, &mflag);
	    } else {
		shift3_(&i1xx, &jyy, &kzz, &mflag);
	    }
...
/* ------------------------------------------------------------------------ */
/* Subroutine */ int shift1_(integer *i1xx, integer *jyy, integer *kzz, 
	integer *mflag)
{
    /* System generated locals */
    integer i__1, i__2, i__3;
	...
	for (data_1.jy = *jyy; data_1.jy <= i__2; ++data_1.jy) {
	    i__3 = *i1xx + 1;
	    for (data_1.i1x = *i1xx; data_1.i1x <= i__3; ++data_1.i1x) {
		chkovlp_(mflag);
...
/* ------------------------------------------------------------------------ */
/* Subroutine */ int shift2_(integer *i1xx, integer *jyy, integer *kzz, 
	integer *mflag)
{
    /* System generated locals */
    integer i__1, i__2, i__3;
    ...
    if (data_1.i1x1 >= 2 && data_1.i1x1 < param_1.nc) {
	i__1 = *kzz + 1;
	for (data_1.kz = *kzz; data_1.kz <= i__1; ++data_1.kz) {
	    i__2 = *jyy + 1;
	    for (data_1.jy = *jyy; data_1.jy <= i__2; ++data_1.jy) {
		i__3 = *i1xx + 1;
		for (data_1.i1x = *i1xx - 1; data_1.i1x <= i__3; ++data_1.i1x)
			 {
		    chkovlp_(mflag);
		    if (*mflag == 1 || index_1.movlpcond == 1) {
			goto L10;
...
/* ------------------------------------------------------------------------ */
/* Subroutine */ int shift3_(integer *i1xx, integer *jyy, integer *kzz, 
	integer *mflag)
{
    /* System generated locals */
    integer i__1, i__2, i__3;
...
/* ------------------------------------------------------------------------ */
/* Subroutine */ int shift4_(integer *i1xx, integer *jyy, integer *kzz, 
	integer *mflag)
{
    /* System generated locals */
    integer i__1, i__2, i__3;
...
/* Subroutine */ int findzmax_(integer *mflag)
{
    ...
    ncntst = sortdim_1.icntst[data_1.i1x + (data_1.jy + data_1.kz * 200) * 
	    200 - 40201];
    if (ncntst == 0) {
	goto L20;
    }
	...
	xr = sortdim_1.xcst[data_1.i1x + (data_1.jy + (data_1.kz + icst * 200)
		 * 200) * 200 - 8040201];
	yr = sortdim_1.ycst[data_1.i1x + (data_1.jy + (data_1.kz + icst * 200)
		 * 200) * 200 - 8040201];
	zr = sortdim_1.zcst[data_1.i1x + (data_1.jy + (data_1.kz + icst * 200)
		 * 200) * 200 - 8040201];
	rr = sortdim_1.rcst[data_1.i1x + (data_1.jy + (data_1.kz + icst * 200)
		 * 200) * 200 - 8040201];
...
L10:
		...
		data_1.x1 += 1e-4f;
		data_1.y1 += 1e-4f;
		if (data_1.x1 >= param_1.cmptln * data_1.i1x1 && data_1.i1x1 <
			 param_1.nc) {
		    data_1.i1x1 = data_1.i1x1;
		}
...
/* Subroutine */ int roll1loc_(void)
{
    ...
    i__1 = ncntst;
    for (icst = 1; icst <= i__1; ++icst) {
	if (! (data_1.i1x == data_1.i1x2 && data_1.jy == data_1.jy2)) {
	    if (! (data_1.kz == data_1.kz2 && icst == data_1.icst2)) {
		calr1loc_(&icst);
...
/* Subroutine */ int roll2loc_(void)
{
	...
	if (! (data_1.i1x == data_1.i1x2 && data_1.jy == data_1.jy2 && 
		...
		data_1.i1x == data_1.i1x3 && data_1.jy == data_1.jy3 && 
...
/* Subroutine */ int register_(void)
{
    ...
    icst1 = sortdim_1.icntst[data_1.i1x1 + (data_1.jy1 + data_1.kz1 * 200) * 
	    200 - 40201] + 1;
    sortdim_1.icntst[data_1.i1x1 + (data_1.jy1 + data_1.kz1 * 200) * 200 - 
	    40201] = icst1;
    sortdim_1.xcst[data_1.i1x1 + (data_1.jy1 + (data_1.kz1 + icst1 * 200) * 
	    200) * 200 - 8040201] = data_1.x1;
    sortdim_1.ycst[data_1.i1x1 + (data_1.jy1 + (data_1.kz1 + icst1 * 200) * 
	    200) * 200 - 8040201] = data_1.y1;
    sortdim_1.zcst[data_1.i1x1 + (data_1.jy1 + (data_1.kz1 + icst1 * 200) * 
	    200) * 200 - 8040201] = data_1.z1;
    sortdim_1.rcst[data_1.i1x1 + (data_1.jy1 + (data_1.kz1 + icst1 * 200) * 
	    200) * 200 - 8040201] = data_1.r1;
/*      write(*,2010) xcst(i1x1,jy1,kz1,icst1), ycst(i1x1,jy1,kz1,icst1), */
/*     1              zcst(i1x1,jy1,kz1,icst1), rcst(i1x1,jy1,kz1,icst1) */
/*      write(*,2020) i1x1,jy1,kz1,icst1 */
/*      write(*,2000) rcst(i1x1,jy1,kz1,icst1), i1x1,jy1,kz1,icst1 */
/* L2000: */
/* L2010: */
/* L2020: */
    return 0;
...
L10:
	...
	data_1.i1x1 += sign_1.lsign;
    }
    if (data_1.i1x1 == 0 || data_1.i1x1 == param_1.nc + 1) {
	data_1.x1 -= param_1.cmptln * 2.f * (real) sign_1.lsign;
	data_1.i1x1 -= sign_1.lsign << 1;
	data_1.r1 *= .9f;
	index_1.mcount = 0;
	index_1.ntshift = 0;
	goto L10;
...
{
    if (data_1.x1 >= param_1.cmptln * data_1.i1x1 && data_1.i1x1 < param_1.nc)
	     {
	++data_1.i1x1;
    }
    if (data_1.x1 < param_1.cmptln * (data_1.i1x1 - 1) && data_1.i1x1 >= 2) {
	--data_1.i1x1;
    }
	 ...
	 real *y0, real *z0, real *rtrack)
{
    ...
    static integer msign, jy1tmp, kz1tmp, i1x1tmp;
    static real x13tort;
    extern /* Subroutine */ int chkcmpt_(void), findxyz_(void);
...
/* 10		END */
/* rnpack.f(2626): error #5082: Syntax error, found INTEGER_CONSTANT '10' when expecting one of: <LABEL> <END-
OF-STATEMENT> ; <IDENTIFIER> TYPE MODULE ELEMENTAL IMPURE NON_RECURSIVE... */
/* ------------------------------------------------------------------------ */
/* Subroutine */ int localshift_(void)
{
    ...
    shift_1.i1xshift[local_1.lshift - 1] = data_1.i1x1;
...
L30:
    ...
    data_1.i1x1 = shift_1.i1xshift[shift_1.indx[i__ - 1] - 1];
...
/* Subroutine */ int finalset_(void)
{
    ...
    static integer ic, jy, kz, i1x, iic, nch, ncl;
    static real ravg;
    ...
    /* Fortran I/O blocks */
    static cilist io___298 = { 0, 7, 0, fmt_2000, 0 };
    static cilist io___299 = { 0, 7, 0, fmt_2010, 0 };
    static cilist io___300 = { 0, 7, 0, fmt_2015, 0 };
    static cilist io___301 = { 0, 5, 0, fmt_1000, 0 };
    static cilist io___310 = { 0, 9, 0, fmt_2020, 0 };
    static cilist io___312 = { 0, 9, 0, fmt_2030, 0 };
    static cilist io___313 = { 0, 7, 0, fmt_2016, 0 };
...
/*     domain */
	...
	for (jy = 1; jy <= i__2; ++jy) {
	    i__3 = param_1.nc;
	    for (i1x = 1; i1x <= i__3; ++i1x) {
		ncntst = sortdim_1.icntst[i1x + (jy + kz * 200) * 200 - 40201]
			;
		i__4 = ncntst;
		for (icst = 1; icst <= i__4; ++icst) {
		    r1 = sortdim_1.rcst[i1x + (jy + (kz + icst * 200) * 200) *
			     200 - 8040201];
...
/* .....Write out the final extracted packing data! */
	...
	for (jy = ncl; jy <= i__2; ++jy) {
	    i__3 = nch;
	    for (i1x = ncl; i1x <= i__3; ++i1x) {
		ncntst = sortdim_1.icntst[i1x + (jy + kz * 200) * 200 - 40201]
			;
		ic = 0;
		i__4 = ncntst;
		for (icst = 1; icst <= i__4; ++icst) {
		    x1 = sortdim_1.xcst[i1x + (jy + (kz + icst * 200) * 200) *
			     200 - 8040201];
		    y1 = sortdim_1.ycst[i1x + (jy + (kz + icst * 200) * 200) *
			     200 - 8040201];
		    z1 = sortdim_1.zcst[i1x + (jy + (kz + icst * 200) * 200) *
			     200 - 8040201];
		    r1 = sortdim_1.rcst[i1x + (jy + (kz + icst * 200) * 200) *
			     200 - 8040201];
		    if (x1 >= param_1.tolrnce && x1 <= param_1.size + 
			    param_1.tolrnce && y1 >= param_1.tolrnce) {
			if (y1 <= param_1.size + param_1.tolrnce) {
			    if (z1 >= param_1.tolrnce) {
				if (z1 <= param_1.height + param_1.tolrnce) {
				    ++param_1.npart;
				    ++ic;
				    origdim_1.x[ic - 1] = x1;
				    origdim_1.y[ic - 1] = y1;
		...
		s_wsfe(&io___310);
		do_fio(&c__1, (char *)&i1x, (ftnlen)sizeof(integer));
