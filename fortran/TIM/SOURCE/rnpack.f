...
c.....Generate the log-normal distribution of particle radii
      ...
      ELSE 
            ...
            ENDDO
			   ...
			   Pd = Vpack/Vt
			   WRITE(*,*)Vpack,Vt,npart
         ENDDO
      ENDIF
		
		WRITE(*,*) 'The maximum radius = ', rmax
		WRITE(*,*) 'The average radius = ', ravg
		WRITE(*,*) 'The sample size = ', size
      WRITE(*,*) 'The sampple height = ', height
		WRITE(*,*) 'The number of spheres in the initial packing = ',
     1           npart
...
c------------------------------------------------------------------------
     ...
     4                 icnt(ncmax,ncmax) 
      ...
      DO i = 1, npart
			CALL RANDOM_NUMBER(arg1)
			CALL RANDOM_NUMBER(arg2)
...
c.....Normalize the distribution
		...
		DO k = 1, mr
			...
			write(12,2000) (rmin+delr*(k-1)), cpdf(k)
      ENDDO
...
c.....Read in the random P-field data
		...
		DO kz = 1, Nzgrid
			DO jy = 1, Ngrid
				DO ix = 1, Ngrid
					READ(4,1010) Pfun(ix,jy,kz)
c    				write(*,*) ix,jy,kz,Pfun(ix,jy,kz)
            ENDDO
         ENDDO
      ENDDO
...
c.....Create the initial random packing with spatial correlations
         ...
         ENDDO
		   ...
		   z(npart) = zsample
			...
			IF(pf.LT.cpdf(1)) THEN
				CALL RANDOM_NUMBER(arg1)
				r(npart) = rmin-delr*arg1
         ELSEIF(pf.GE.cpdf(mr)) THEN
				CALL RANDOM_NUMBER(arg1)
				r(npart) = rmax+delr*arg1
         ELSE
			   DO k = 1, mr-1
			   	IF(pf.GE.cpdf(k).AND.pf.LT.cpdf(k+1)) THEN
			   		CALL RANDOM_NUMBER(arg1)
			   		r(npart) = rmin+delr*(k-1)+delr*arg1
               ENDIF
            ENDDO
			ENDIF
			Vpack = Vpack+4.0*pi*r(npart)*r(npart)*r(npart)/3.0
     ...
     1           npart
      ...
      bound = cmptln-2.0*rmax
		...
		WRITE(*,*) 'Start rearranging process(1--Yes, 0--No)?'
		READ(*,*) lsignal
...
c     WRITE(8,2000) 
c		WRITE(8,*) (x(i), y(i), z(i), r(i), i = 1, npart)
...
c------------------------------------------------------------------------
     ...
     1               npart_kz, pi, porosity, rlmt, nrndsd, size, 
     2               theta, tolrnce, ncz
		COMMON /data/ icst2, icst3, icst4, ix, ix1, ix2, ix3, ix4,
...
c.....Sum up the total counts in the layer
			...
			DO jy = 1, nc
				DO ix = 1, nc
					npart_kz = npart_kz+icnt(ix,jy)
            	ENDDO
         	ENDDO

c.....Write particles in file after allocation

			WRITE(9,2010) npart_kz
			DO jy = 1, nc
            DO ix = 1, nc
               ncpart = icnt(ix,jy)
					WRITE(9,2020) ix, jy, ncpart
c					WRITE(9,*) (xc(ix,jy,ic), yc(ix,jy,ic), zc(ix,jy,ic), 
c     1                     rc(ix,jy,ic), ic = 1, ncpart) 
				   DO ic = 1, ncpart
						WRITE(9,2030) xc(ix,jy,ic), yc(ix,jy,ic),
     1                          zc(ix,jy,ic), rc(ix,jy,ic)
					ENDDO
				ENDDO
         ENDDO
			mz = mz+1
...
2000  FORMAT('npart_kz',/,'ix  jy  ncpart',/,'xc  yc  zc  rc')
2010  FORMAT(i7)
2020  FORMAT(3i7)
		...
		DO lz = 1, ncz
		   READ(9,*) npart_kz
			DO jy = 1, nc
               ...
               icnt(ix,jy) = ncpart
               DO ic = 1, ncpart
						READ(9, 1010) xc(ix,jy,ic), yc(ix,jy,ic),
     1                          zc(ix,jy,ic), rc(ix,jy,ic)
               ENDDO
				ENDDO
         ENDDO

   		WRITE(*,*) 'Working on the vertical cell: ', lz
...
c------------------------------------------------------------------------
     ...
     4                 icnt(ncmax,ncmax) 
      ...
      icnt(ix1,jy1) = icnt(ix1,jy1)-1
		DO ic = ic1+1, ncpart1
			xc(ix1,jy1,ic-1) = xc(ix1,jy1,ic)
			yc(ix1,jy1,ic-1) = yc(ix1,jy1,ic)
			zc(ix1,jy1,ic-1) = zc(ix1,jy1,ic)
			rc(ix1,jy1,ic-1) = rc(ix1,jy1,ic)
      ENDDO
...
c------------------------------------------------------------------------
     ...
     1               niterate
      ...
      mflag = 1
		...
		DO WHILE(mflag.EQ.1)
			IF(ix1.EQ.nc) ixx = nc-1
			IF(ix1.LT.nc) ixx = ix1
         ...
         IF(kz1.LT.(ncz+1)) kzz = kz1
			...
			IF(kz1.EQ.1.OR.kz1.EQ.(ncz+1)) THEN
				IF(ix1.EQ.1.AND.jy1.EQ.1.OR.ix1.EQ.nc.AND.jy1.EQ.1.OR.
     1         ix1.EQ.1.AND.jy1.EQ.nc.OR.ix1.EQ.nc.AND.jy1.EQ.nc) THEN
               CALL Shift1(ixx,jyy,kzz,mflag)
            ELSEIF(ix1.EQ.1.OR.ix1.EQ.nc.OR.jy1.EQ.1.OR.jy1.EQ.nc) THEN
					CALL Shift2(ixx,jyy,kzz,mflag)
            ELSE
					CALL Shift3(ixx,jyy,kzz,mflag)
            ENDIF
         ELSEIF(jy1.EQ.1.OR.jy1.EQ.nc) THEN
            IF(ix1.EQ.1.OR.ix1.EQ.nc) THEN
					CALL Shift2(ixx,jyy,kzz,mflag)
            ELSE
					CALL Shift3(ixx,jyy,kzz,mflag)
            ENDIF
         ELSEIF(ix1.EQ.1.OR.ix1.EQ.nc) THEN
				CALL Shift3(ixx,jyy,kzz,mflag)
         ELSE
				CALL Shift4(ixx,jyy,kzz,mflag)
         ENDIF
      ENDDO
...
c------------------------------------------------------------------------
      ...
      SUBROUTINE Shift1(ixx,jyy,kzz,mflag)
...
c------------------------------------------------------------------------
      ...
      SUBROUTINE Shift2(ixx,jyy,kzz,mflag)
     ...
     1               niterate
      ...
      IF(ix1.GE.2.AND.ix1.LT.nc) THEN
			DO kz = kzz, kzz+1
				DO jy = jyy, jyy+1
					DO ix = ixx-1, ixx+1
						CALL ChkOvlp(mflag)
...
c------------------------------------------------------------------------
      ...
      SUBROUTINE Shift3(ixx,jyy,kzz,mflag)
...
c------------------------------------------------------------------------
      ...
      SUBROUTINE Shift4(ixx,jyy,kzz,mflag)
...
c------------------------------------------------------------------------
     ...
     4                 icntst(ncmax,ncmax,ncmax) 
		...
		DO icst = 1, ncntst
			xr = xcst(ix,jy,kz,icst)
			yr = ycst(ix,jy,kz,icst)
			zr = zcst(ix,jy,kz,icst)
			rr = rcst(ix,jy,kz,icst)
...
10          dxy = SQRT((x1-xr)*(x1-xr)+(y1-yr)*(y1-yr))
               ...
               x1 = x1+1.0e-4
               y1 = y1+1.0e-4
					IF(x1.GE.cmptln*ix1.AND.ix1.LT.nc) ix1 = ix1+1
...
c------------------------------------------------------------------------
     ...
     1               niterate
      ...
      DO icst = 1, ncntst
			IF(.NOT.(ix.EQ.ix2.AND.jy.EQ.jy2.AND.kz.EQ.kz2.AND.
...
c------------------------------------------------------------------------
     ...
     1               niterate
      ...
      DO icst = 1, ncntst
			IF((.NOT.(ix.EQ.ix2.AND.jy.EQ.jy2.AND.kz.EQ.kz2.AND.
     1      icst.EQ.icst2)).AND.(.NOT.(ix.EQ.ix3.AND.
...
c------------------------------------------------------------------------
     ...
     1               niterate
      ...
      icst1 = icntst(ix1,jy1,kz1)+1
      icntst(ix1,jy1,kz1) = icst1
      xcst(ix1,jy1,kz1,icst1) = x1
      ycst(ix1,jy1,kz1,icst1) = y1
      zcst(ix1,jy1,kz1,icst1) = z1
      rcst(ix1,jy1,kz1,icst1) = r1

c      write(*,2010) xcst(ix1,jy1,kz1,icst1), ycst(ix1,jy1,kz1,icst1),
c     1              zcst(ix1,jy1,kz1,icst1), rcst(ix1,jy1,kz1,icst1)
c      write(*,2020) ix1,jy1,kz1,icst1

c      write(*,2000) rcst(ix1,jy1,kz1,icst1), ix1,jy1,kz1,icst1

c...   MH 07/18/99: the old format definitions make trouble when compiling
c...                on the ATN SGI. I suspect they are not used but to be 
c...                sure I replace them by "RUBBISH".
c...                If this change was not good RUBBISH should appear 
c...                somewhere in the output files 
c 2000	FORMAT(f8.5, 1x, 4(i3,ix))
2000	FORMAT(f8.5, 1x, 4(i3,'RUBBISH'))
2010	FORMAT(4(e13.5,2x))
c 2020	FORMAT(4(i3,ix))
2020	FORMAT(4(i3,'RUBBISH'))
     ...
     1               niterate
      ...
      IF(mcount.EQ.10) THEN
			IF(mark.EQ.1) THEN
				IF(x1.LT.bound) THEN
					x1 = x1+cmptln
					IF(x1.GE.cmptln) ix1 = ix1+1
            ELSE
					x1 = x1-cmptln
					IF(x1.LT.(size-cmptln)) ix1 = ix1-1
            ENDIF
	      ELSE				
				IF(y1.LT.bound) THEN
					y1 = y1+cmptln
...
10    IF(mcount.EQ.0) THEN
     ...
     1      (y1.LT.bound.OR.y1.GT.(size-bound))) THEN
      ...
      ENDIF
		...
		IF(ix1.EQ.0.OR.ix1.EQ.(nc+1)) THEN
...
c------------------------------------------------------------------------
     ...
     3              y1, y2, y3, y4, z1, z2, z3, z4 
      ...
      IF(x1.GE.(cmptln*ix1).AND.ix1.LT.nc) ix1 = ix1+1
		...
		IF(x1.LT.(cmptln*(ix1-1)).AND.ix1.GE.2) ix1 = ix1-1
...
c------------------------------------------------------------------------
     ...
     3              y1, y2, y3, y4, z1, z2, z3, z4 
      ...
      COMMON /index/ mcount, movlpcond, mpBC, mroll, nrec, ntshift,
     1               niterate
      COMMON /shift/ xshift(30), yshift(30), zshift(30), ixshift(30),
      ...
      kzshift(lshift) = kz1
		...
		IF(lshift.LT.move) THEN
			...
			DO WHILE(sample.GE.(size-bound).OR.sample.LE.bound)
				CALL RANDOM_NUMBER(arg1)
				sample = x1-rmax+ABS(2.0*rmax*arg1)
         ENDDO
			x1 = sample
			...
			DO WHILE(sample.GE.(size-bound).OR.sample.LE.bound)
				CALL RANDOM_NUMBER(arg1)
				sample = y1-rmax+ABS(2.0*rmax*arg1)
         ENDDO
			y1 = sample
...
c     domain
     ...
     1              npart, nc, ncz, mpBC
		WRITE(7,2010) alphar,betar, rmax, rmin, ravg, rlmt
		WRITE(7,2015) niterate, intensity
...
c.....Write out the final extracted packing data!
     ...
     2               z1.GE.tolrnce.AND.z1.LE.(height+tolrnce)) THEN
                  ...
                  ENDIF
               ENDDO
C					WRITE(9,2020) ix, jy, kz, ic
					DO iic = 1, ic
						WRITE(9,2030) x(iic), y(iic), z(iic), r(iic)
               ENDDO
            ENDDO
         ENDDO
      ENDDO

		WRITE(7,2016) npart, ncl, nch, nczl, nczh
